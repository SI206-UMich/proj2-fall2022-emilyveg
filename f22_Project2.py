...
.
    ...
    #opening file & creating soup object
    file = open(html_file)
    ...
    #find the anchor tags (a's)
    title_tag = soup.find_all('a',class_ = 'ln2bl2p dir dir-ltr')
    pattern = 'listing_(.)*'
    
    #grab the target attribute of each anchor tag
    for x in title_tag:
        if re.search(pattern, str(x)):
            a_tags.append(x.get('target'))
    # print(a_tags)

    #loop through listings to make just a list of listing ids
    for listing in a_tags:
    ...
    for id in id_nums:
        ...
        # listing_title = soup.find('div', class_= "t1jojoys dir dir-ltr")
        # print(listing_title)
        ...
        cost1 = soup.find('span', class_ = "_tyxjp1")
        cost = cost1.text[1:]

        tup = (final_t , int(cost), id)
        # print(tup)
...
.
    ...
    #opening file & creating soup object
    file = open(f'html_files/listing_{listing_id}.html')
    soup = BeautifulSoup(file, 'html.parser')
    file.close()
...
def check_policy_numbers(data):
    ...
    for x in data:
        # print(x[3])
        if x[3] != re.search(pattern1, str(x)) or (pattern2, str(x)):
            no_match.append(x)
        elif x[3] == "Pending" or "Exempt":
            x[3] = x[3]
        else:
            x[3] =x[3]
        # print(no_match)
...
class TestCases(unittest.TestCase):
    ...
    def test_get_listing_information(self):
        ...
        self.assertEqual(html_list[0], 'STR-0001541')

        # check that the last listing in the html_list is a "Private Room"
        self.assertEqual(html_list[-1][1], 'Private Room')

        # check that the third listing has one bedroom
        self.assertEqual(html_list[2][2], '1')
    ...
    def test_check_policy_numbers(self):
        ...
        self.assertEqual(type(detailed_database), str)
